...
if(!function_exists('parcelStatus')){
    function parcelStatus($parcel){
        ...
        elseif($parcel->status == parcelStatus::PARTIAL_DELIVERED){
            $allowStatus = [parcelStatus::PARTIAL_DELIVERED_CANCEL,ParcelStatus::RETURN_TO_COURIER];
        }
...
if (!function_exists('parcelExpense')) {
    function parcelExpense($id)
    {
        ...
        // return DeliverymanStatement::where('parcel_id',$id)->where('type',AccountHeads::INCOME)->sum('amount');
...
if (!function_exists('parcelExpenseTotal')) {
   ...
   {
        //   return DeliverymanStatement::whereIn('parcel_id',$ids)->where('type',AccountHeads::INCOME)->sum('amount');
...
if (!function_exists('parcelsStatus')) {
    ...
    if (!function_exists('notifications')) {
        ...
        function notifications()
        {
            ...
            $notifications     = [];
            //suports
            $supports          = Support::orderBy('created_at', 'DESC')->select('id','user_id','subject','created_at')->limit(5)->get();
            ...
            $supportsChat  = SupportChat::with(['support'])->orderBy('created_at','DESC')->limit(5)->get();
            foreach ($supportsChat as  $chat) {
                $notifications []= [
                    'type'      => 'support',
