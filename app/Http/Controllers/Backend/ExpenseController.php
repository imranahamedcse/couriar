...
class ExpenseController extends Controller
{
    ...
    public function store(StoreExpenseRequest $request)
    {
        ...
        if($result == 2){
            toast(__('expense.not_enough_balence'),'warning');
            return redirect()->route('expense.index');
        }
        if($result == 1){
            toast(__('expense.added_msg'),'success');
            return redirect()->route('expense.index');
        }
        else{
            toast(__('expense.error_msg'),'error');
    ...
    public function update($id, UpdateExpenseRequest $request)
    {
        ...
        if($result == 2){
            ...
            toast(__('expense.update_msg'),'success');
            return redirect()->route('expense.index');
        }else{
            toast(__('expense.error_msg'),'error');
            return redirect()->back();
    ...
    public function destroy($id)
    {
        if($this->repo->delete($id)){
            toast(__('expense.delete_msg'),'success');
            return back();
        }
        else{
            toast(__('expense.error_msg'),'error');
