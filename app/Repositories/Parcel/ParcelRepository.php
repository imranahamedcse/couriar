...
class ParcelRepository implements ParcelInterface {
        ...
        try {
            ...
            if(SmsSendSettingHelper(SmsSendStatus::PARCEL_CREATE)) {
               ...
               if(session()->has('locale') && session()->get('locale') == 'bn'):
                    ...
                    $msg = 'Dear '.$parcel->customer_name.', Your parcel is successfully created. Your parcel with ID ' . $parcel->tracking_id . ' parcel from ' . $parcel->merchant->business_name . ' (TK' . $parcel->cash_collection . ')';
                endif;
        ...
        try {
            ...
            if($request->send_sms_merchant  == 'on'){
                ...
                if(session()->has('locale') && session()->get('locale') == 'bn'):
                    ...
                    $msg = 'Dear '.$parcel->merchant->business_name.', your  parcel with ID '.$parcel->tracking_id .' Pickup man assign from RX courier. Assign by '.$pickupAsisgn->pickupman->user->name.', '.$pickupAsisgn->pickupman->user->mobile.' Track here: '.url('/').' -'.settings()->name;
                    $response =  app(SmsService::class)->sendSms($parcel->merchant->user->mobile,$msg);
                endif;
        ...
        try {
            ...
            if($request->send_sms_merchant  == 'on'){
                if(session()->has('locale') && session()->get('locale') == 'bn'):
                    ...
                    $msg = 'Dear'.$parcel->merchant->business_name.', your  parcel with ID '.$parcel->tracking_id .' Pickup man assign from RX courier. Assign by'.$pickupReshcedule->pickupman->user->name.', '.$pickupReshcedule->pickupman->user->mobile.' Track here: '.url('/').' -'.settings()->name;
                    $response =  app(SmsService::class)->sendSms($parcel->merchant->user->mobile,$msg);
                endif;
    ...
    public function returnAssignToMerchant($id,$request){
        try {
            ...
            // Courier statemnt when parcel returned to merchant
            // Start
    ...
    public function returnAssignToMerchantRescheduleCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::RETURN_MERCHANT_RE_SCHEDULE){
                ...
                // ParcelEvent::destroy($merchantReschedule->id);
            }
        ...
        try {
            ...
            $parcel->cash_collection            = $parcel->old_cash_collection;
             //calculations
                ...
                // dd($cod_charges_amount,$total_charges,$vat_amount,$chargeWithVat,$totaldeliveryAmount);
    ...
    public function PickupReScheduleCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::PICKUP_RE_SCHEDULE){
                ...
                // ParcelEvent::destroy($pickupReschedule->id);
    ...
    public function deliveryReScheduleCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::DELIVERY_RE_SCHEDULE ){
                ...
                // ParcelEvent::destroy($deliverymanReschedule->id);
            }
            $parcel->status   = ParcelStatus::DELIVERY_MAN_ASSIGN;
    ...
    public function transfertoHubCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::TRANSFER_TO_HUB ){
                ...
                // ParcelEvent::destroy($transfertohub->id);
            }
    ...
    public function receivedByHubCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::RECEIVED_BY_HUB ){
                ...
                // ParcelEvent::destroy($receivedByhub->id);
            }
